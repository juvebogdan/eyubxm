...
<tbody>
<?php foreach($nightwork_table as $row):?>
<tr>
...
<?php if($row->status == 0):?>
<td class="text-center"><a href="<?php echo base_url('bxmtime/deletenight/'. $row->id . '/' . $row->identifier);?>" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></a>&nbsp;</td>
<?php else:?>
<td class="text-center"><a href="<?php echo base_url('bxmtime/deletenight/'. $row->id . '/' . $row->identifier);?>" class="btn btn-danger btn-sm disabled"><span class="glyphicon glyphicon-trash"></span></a>&nbsp;</td>                        
<?php endif;?>
</tr>
<?php endforeach;?>
</tbody> 