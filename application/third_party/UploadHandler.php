...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null, $uploadtarget) {
        $this->response = array();
        $this->uploadtarget = $uploadtarget;
        $this->options = array(
            'script_url' => $this->get_full_url().'/'.$this->basename($this->get_server_var('SCRIPT_NAME')),
            'upload_dir' => $uploadtarget,
    ...
    protected function handle_form_data($file, $index) {
        $ci =& get_instance();
        $ci->load->model('preventivemodel');
        $lokacija = $ci->preventivemodel->get_sitecode($_POST['id']);

        if ($_POST['date']=='') {
            exit('Morate odabrati datum');        
        } 

        $datum = DateTime::createFromFormat('d/m/Y', $_POST['date']);
        $datum = date_format($datum,"Ymd");    

        $this->options['upload_dir'] = 'Maintenance/' . $lokacija[0]->sitecode . '/Maintenance/' . $lokacija[0]->tip . '/' . $datum . '/Photo/Actual/';
    }
